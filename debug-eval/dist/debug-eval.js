...
(() => {
  ...
  var LLMCacheStorage = import_localforage.default.createInstance({
  ...
  function extractAllPlainChunks(text, minLength = 30) {
    return text.split(/\[.*?\]|<[^>]*>|\{\{.*?\}\}/gs).map((s) => s.trim()).filter((s) => s.length >= minLength).sort((a, b) => b.length - a.length);
  }
  window.debugPlugin = {
    // 캐시 통계
    async cacheStats() {
...
Sample key length: ${sampleKey.length} chars`);
    ...
    // 텍스트가 포함된 캐시 키 찾기
    async findKeysContaining(text, limit = 10) {
      const keys = await LLMCacheStorage.keys();
      const matches = [];
    ...
    // 캐시 키 검색 성능 테스트
    async benchmarkSearch(searchText) {
    ...
    async eval(code) {
      try {
        ...
        return await fn(LLMCacheStorage, import_localforage.default, getDatabase, getChar);
      } catch (e) {
...
All chunks:`);
    ...
    async findCacheByChunk(text) {
      ...
      if (matches.length === 1) {
        const value2 = await LLMCacheStorage.getItem(matches[0]);
        console.log(`Single match found!`);
        return { type: "chunk", key: matches[0], value: value2 };
      }
      console.log(`Multiple matches - selecting by length similarity`);
      const originalLength = text.length;
      matches.sort(
        (a, b) => Math.abs(a.length - originalLength) - Math.abs(b.length - originalLength)
      );
